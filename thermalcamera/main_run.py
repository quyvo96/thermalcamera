            ...
            if(PG.flag_get_temp != 4):
                ...
                if(PG.flag_get_temp == 3):
                    ...
                    if(time.time() - PG.Previous_Time2 >= 7.2):
                        ...
                        PG.Previous_Time2 = time.time()
                if(faceTempRaw != 0.0 ):
                    if(PG.TempEnvirDefault > 0 and PG.envirFirst25 > 0):
                        PG.calibFaceAve = round (AddCalc(PG.envirFirst25, PG.TempEnvirDefault, float(PG.values2), float(PG.values6), float(PG.values7)),1) 
...
class face_recognition_frame(frame_main1):
    ...
    def m_panel_videoOnPaint(self,event):
        ...
        if(PG.flag_get_temp == 3):
            ...
            if(float(PG.showTopofScreen) == 0.0 and PG.normalTemp == 0):
                ...
                if(PG.Temp_text_backup >= float(PG.values0)):
                    ...
                    PG.normalTemp = 1
                    PG.timeNormalTemp = time.time()
                    self.m_textCtrl_temp.SetValue("体温は正常です")#Normal temperature
            if(PG.normalTemp == 1):
                self.m_textCtrl_temp.SetValue("体温は正常です")#Normal temperature
                if(time.time() - PG.timeNormalTemp >= 3):
                    self.m_textCtrl_temp.SetValue(str(PG.Temp_text_on_top))
                    PG.normalTemp = 0
...
def AddCalc(Tdef, Wave, Bias20, Bias2023, Bias23):
    global value
    if(Tdef <= PG.TempLevelLow):
        if(Wave<=PG.TempLevelLow):
            PG.faceChange = "Low20"
            value = (Tdef - Wave) * Bias20
        elif(Wave > PG.TempLevelLow and Wave < PG.TempLevelHigh):
            PG.faceChange = "Low2023"
            value = (PG.TempLevelLow - Wave) * Bias20 + (Tdef - PG.TempLevelLow) * Bias2023
        else:
            PG.faceChange = "Low23"
            value = (PG.TempLevelLow - Wave) * Bias20 + 3 * Bias2023 + (Tdef - PG.TempLevelHigh) * Bias23
    if(Tdef > PG.TempLevelLow and Tdef < 23):
        if(Wave<=PG.TempLevelLow):
            PG.faceChange = "Mid20"
            value = (Tdef - PG.TempLevelLow) * Bias20 + (PG.TempLevelLow - Wave) * Bias2023
        elif(Wave > PG.TempLevelLow and Wave < PG.TempLevelHigh):
            PG.faceChange = "Mid2023"
            value = (Tdef - Wave) * Bias2023
        else:
            PG.faceChange = "Mid23"
            value = (Tdef - PG.TempLevelHigh) * Bias2023 +  (PG.TempLevelHigh - Wave)* Bias23
    if(Tdef >= PG.TempLevelHigh):
        if(Wave<=PG.TempLevelLow):
            PG.faceChange = "High20"
            value = (Tdef - PG.TempLevelLow) * Bias20 + (PG.TempLevelLow - PG.TempLevelHigh) * Bias2023 + (PG.TempLevelHigh - Wave) * Bias23
        elif(Wave > PG.TempLevelLow and Wave < PG.TempLevelHigh):
            PG.faceChange = "High2023"
            value = (PG.TempLevelHigh - Tdef) * Bias2023 + (Tdef - Wave) * Bias23
        else:
            PG.faceChange = "High23"
            value = (Tdef - Wave) * Bias23
    return value
def camera(frame):
...
class dialog_setting(MyDialog_setting):
    ...
    def __init__(self, parent):
        ...
        ##########
        self.m_textCtrl_levelLow.SetValue("<="+str(PG.TempLevelLow)+"°C:")
        self.m_textCtrl_levelMid.SetValue(str(PG.TempLevelLow)+"to"+str(PG.TempLevelHigh)+"°C:")
        self.m_textCtrl_levelHigh.SetValue(">="+str(PG.TempLevelHigh)+"°C:")
        #########
        if(PG.setlog== 1):
